...
// Înlocuim doar lista de nume de liste (mockuită)
const mockedListsNames = [
  { id: 1, name: "Lista 1" },
  { id: 2, name: "Lista 2" },
  { id: 3, name: "Lista 3" },
...
const fetchDrinks = async () => {
  ...
  try {
    const response = await fetch(`http://localhost:3000/drinks/ranking?limit=100`, {
      headers: {
        Authorization: `Bearer ${token}`
      }
    });
    if (!response.ok) throw new Error("Eroare la încărcarea băuturilor");
    const data = await response.json();

    // Normalizăm pentru a păstra compatibilitatea cu logica existentă
    return data.map((drink) => ({
      id: drink.id,
      name: drink.name,
      category: drink.category || "necunoscut",
      quantity: drink.quantity || 0,
      listCount: drink.favoritesCount || 0,
      image: drink.image_url || "",
    }));
  } catch (err) {
    console.error("Eroare API ranking:", err);
    return [];
  }
};
    ...
    <div class="drink-details">
      ...
      <div class="drink-content">
        <div class="drink-text">
          <p id="drink-category"><strong>Categorie:&nbsp;</strong> ${drink.category}</p>
          <p id="drink-category"><strong>Cantitate:&nbsp;</strong> ${drink.quantity || 0} Litri</p>
          <p id="drink-description">Această băutură este una dintre cele mai populare în rândul utilizatorilor, fiind adăugată frecvent în listele favorite.</p>
        </div>
        <div id="icons-container">
          <img id="drink-image-box" src="${drink.image}" alt="${drink.name}">
          <div id="menu-icons-container">
...
function closeAllModals() {
  document
    .querySelectorAll(".text-box:not(.hidden)")
    .forEach((m) => m.classList.add("hidden"));
  overlay.classList.add("hidden");
  body.classList.remove("no-scroll");
}
