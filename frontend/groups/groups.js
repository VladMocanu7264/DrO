...
async function fetchGroupsFromAPI() {
  try {
    const res = await fetch(`${window.env.API_BASE_URL}/groups/user`, { headers });
    if (!res.ok) throw new Error("Grupuri API fail");
...
async function fetchPostsForGroup(groupId) {
  try {
    const res = await fetch(`${window.env.API_BASE_URL}/groups/${groupId}/posts`, { headers });
    if (!res.ok) throw new Error("Postări API fail");
    const data = await res.json();
    return data;
  } catch {
    const group = mockedGroups.find(g => g.id == groupId);
    return mockedPosts.filter(p => group?.posts.includes(p.id));
  }
}

async function handleLikePost(postId) {
  try {
    const res = await fetch(`${window.env.API_BASE_URL}/posts/${postId}/like`, {
      method: "POST",
      headers
    });
    if (!res.ok) throw new Error("Already liked or error");
    await handleSelectGroup(); // Refresh
  } catch {
    const post = mockedPosts.find(p => p.id === postId);
    if (post) {
      post.likes += 1;
      handleSelectGroup(); // Refresh mock
    }
  }
}
...
async function handleSelectGroup() {
  ...
  const posts = await fetchPostsForGroup(selectedGroupId);
  generatePosts(posts);
}

function generateRadioFilter(groups) {
  const container = document.querySelector('.groups-list');
  if (!container) return;
  container.innerHTML = '';
  ...
  groups.forEach((group) => {
    ...
    if (selectedGroupId == group.id) input.checked = true;
...
function generatePosts(posts) {
  ...
  if (posts.length === 0) {
    ...
    empty.className = "empty-post";
    empty.innerHTML = `<h2>Acest grup nu are încă postări.</h2>`;
  ...
  posts.forEach((post) => {
    ...
    postDiv.innerHTML = `
      <div class="post-content">
        <div class="user-info">
          <img src="${post.member?.image_path || post.image}" alt="${post.member?.username || post.drink}" class="user-image">
          <p class="card-user">${post.member?.username || post.user}</p>
        </div>
        <p class="post-text">${post.text}</p>
        <div class="drink-info">
          <span class="number-of-likes">
            <i class="fa-solid fa-heart like-icon"
              onclick="handleLikePost('${post.id}')" title="Like"></i> 
            ${post.likes}
          </span>
          <span class="drink-title">Băutură: ${post.drink?.name || post.drink}</span>
          <span class="post-date">Data: ${new Date(post.time || post.date).toLocaleDateString()}</span>
        </div>
      </div>
      <img src="${post.drink?.image_url || post.image}" alt="${post.drink?.name || post.drink}" class="drink-image">
    `;
    postsContainer.appendChild(postDiv);
  });
}
...
async function render() {
  originalGroups = await fetchGroupsFromAPI();
  generateRadioFilter(originalGroups);
  if (originalGroups.length) {
    selectedGroupId = originalGroups[0].id;
    await handleSelectGroup();
  }
}
